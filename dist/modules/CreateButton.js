...
function createAddAnswerButton() {
    ...
    if (element1 != null) {
        ...
        /* 創建網路假期正確答案陣列 */
        const AnswerArray = []; //宣告陣列
        for (let i = 0; i < linkList.length; i++) {
            if (linkList[i].getElementsByClassName('ans-check')[0] != null) {
                AnswerArray[i] = linkList[i].getElementsByClassName('ans-dot')[0]; //取得網路假期正確答案陣列
            }
            else {
        ...
        /* 將 A B C D E 選項轉換為數字 */
        const NumberArray = AnswerArray.map((answer) => AnswerNumberMap[answer.innerText.toUpperCase()]); //宣告陣列
        if (element2 != null && title != null && NumberArray != null) {
            ...
            ButtonCreate.onclick = function () {
                ...
                const getAnswer = document
                    .getElementsByClassName('radio')[NumberArray[number - 1]].getElementsByTagName('label')[0]
                ...
                if (topic === '.' || topic === ' ' || topic === '') {
                    ...
                    window.open(requestURL +
                        ...
                        '&Answer=' +
                        encodeURIComponent(getAnswer.innerText.replace(/(^\s*)|(\s*$)/g, '').slice(2)) +
                        '&Source=Plugin', '_blank');
                }
                else {
